...
class Seq2Seq(nn.Module):
    ...
    def generate(self, hidden, maxlen, sample=True, temp=1.0):
        ...
        inputs = torch.cat((embedding, hidden.unsqueeze(1)), dim = 2)
        ...
        for i in range(maxlen):
            ...
            '''
            adding indices.unsqueeze(1), original indices
            '''
            all_indices.append(indices.unsqueeze(1))

            embedding = self.embedding_decoder(indices)
            '''
            adding embedding.unsqueeze(1). original had embedding
            '''
            inputs = torch.cat((embedding.unsqueeze(1), hidden.unsqueeze(1)), dim = 2)
